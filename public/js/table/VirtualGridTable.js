(() => {
  // src/client/table/VirtualGridTable.ts
  class VirtualGridTable {
    ...
    constructor(containerId, options = {}) {
      ...
      this._view = null;
      this._viewCount = 0;
      this._mode = this._opts.mode === "server" ? "server" : "local";
      this._chunkRows = new Map;
      this._chunkPending = new Set;
      this._chunkSize = this._clamp(Math.floor(Number(this._opts.pageSize) || 250), 25, 5000);
      this._chunkTotalRows = Math.max(0, Number(this._opts.totalRows) | 0);
      this._onChunkRequest = typeof this._opts.onServerPageRequest === "function" ? this._opts.onServerPageRequest : null;
      this._fetchChunk = typeof this._opts.fetchPage === "function" ? this._opts.fetchPage : null;
      this._chunkSeq = 0;
    ...
    setColumnFilter(colIndex, filterSpec) {
      ...
      if (abs < 0 || abs >= this._columns.length)
        return;
    ...
    setServerPaging(config = {}) {
      const next = config && typeof config === "object" ? config : {};
      if (typeof next.pageSize === "number" && Number.isFinite(next.pageSize)) {
        this._chunkSize = this._clamp(Math.floor(next.pageSize), 25, 5000);
      }
      if (typeof next.onServerPageRequest === "function")
        this._onChunkRequest = next.onServerPageRequest;
      if (typeof next.fetchPage === "function")
        this._fetchChunk = next.fetchPage;
      ...
      this._columnFilters.clear();
      this._closeFilterMenu();
      this.setServerRowCount(next.totalRows ?? 0, false);
      this._resetPipelineState();
      this._rebuildBodyPool();
      this._recomputeView();
    ...
    setServerRows(startIndex, rows, totalRows) {
      if (this._mode !== "server")
        return;
      const start = Math.max(0, startIndex | 0);
      const incoming = Array.isArray(rows) ? rows : [];
      if (Number.isFinite(totalRows))
        this.setServerRowCount(totalRows, false);
    ...
    setServerRowCount(totalRows, rerender = true) {
      this._chunkTotalRows = Math.max(0, totalRows | 0);
      for (const index of Array.from(this._chunkRows.keys())) {
        if (index >= this._chunkTotalRows)
          this._chunkRows.delete(index);
      }
    ...
    _build() {
      ...
      const sUp = this._createButton("vgt__sbtn vgt__triBtn vgt__triBtn--up", "", () => this._scrollBy(-this._bodyH * 0.9), "Scroll up");
      const sDown = this._createButton("vgt__sbtn vgt__triBtn vgt__triBtn--down", "", () => this._scrollBy(this._bodyH * 0.9), "Scroll down");
      this._sUp = sUp;
      this._sDown = sDown;
      ...
      const pUp = this._createButton("vgt__pill vgt__navBtn vgt__triBtn vgt__triBtn--up", "", () => this._scrollBy(-this._bodyH), "Page up");
      const pDown = this._createButton("vgt__pill vgt__navBtn vgt__triBtn vgt__triBtn--down", "", () => this._scrollBy(this._bodyH), "Page down");
      const pLeft = this._createButton("vgt__pill vgt__navBtn vgt__triBtn vgt__triBtn--left", "", () => this._scrollXBy(-this._bodyW * 0.9), "Scroll left");
      const pRight = this._createButton("vgt__pill vgt__navBtn vgt__triBtn vgt__triBtn--right", "", () => this._scrollXBy(this._bodyW * 0.9), "Scroll right");
      this._pLeft = pLeft;
      this._pRight = pRight;
      this._pUp = pUp;
      this._pDown = pDown;
      ...
      const clearFilterBtn = this._createButton("vgt__pill vgt__filterClear", "Clear", () => {
        ...
        if (this._sort && this._sort.colIndex === absCol)
          this.clearSort();
      });
    ...
    _bindEvents() {
      this._mid.addEventListener("wheel", (event) => {
    ...
    _rebuildBodyPool() {
      ...
      if (poolRows === this._renderRows && this._rowEls?.length && this._rowEls[0]?.cellEls.length === colCount) {
        return;
      }
    ...
    _recomputeView() {
      ...
      if (!hasFilter && !hasSearch && !hasSort && !hasColumnFilters)
        return;
    ...
    _rowPassesColumnFilters(row) {
      ...
      for (const [colIndex, filterSpec] of this._columnFilters.entries()) {
        if (!this._matchesColumnFilter(source[colIndex], filterSpec))
          return false;
      }
      return true;
    }
    ...
    _rowSearchStr(baseIndex) {
      ...
      for (let i = 0;i < row.length; i += 1) {
        ...
        if (value == null)
          continue;
        out += String(value).toLowerCase() + "\x01";
      }
    ...
    _viewIndexToBase(viewIndex) {
      if (viewIndex < 0 || viewIndex >= this._viewCount)
        return -1;
    ...
    _ensureColWidths(reset = false) {
      ...
      if (reset || this._colWidths.length !== colCount) {
        ...
        for (let i = 0;i < colCount; i += 1) {
          ...
          const width = Number.isFinite(incoming) ? incoming : Number.isFinite(prior) ? prior : Number.isFinite(auto) ? auto : defaultWidth;
    ...
    _computeAutoColumnWidthsFromLocalData() {
      ...
      if (colCount <= 0 || !Array.isArray(this._rows))
        return null;
    ...
    _cellChromeWidthPx() {
      const padVar = this._root && window.getComputedStyle ? window.getComputedStyle(this._root).getPropertyValue("--vgt-pad-x") : "";
    ...
    _columnTemplate() {
      if (this._colWidths.length === 0)
        return `${this._minColWidth}px`;
      return this._colWidths.map((width) => `${width}px`).join(" ");
    }
    _totalContentWidth() {
      let width = 0;
      for (let i = 0;i < this._colWidths.length; i += 1)
      ...
      if (!this._hcells || this._hcells.length !== colCount) {
        ...
        for (let slot = 0;slot < colCount; slot += 1) {
          ...
          hc.addEventListener("pointerdown", (event) => {
            if (event.target instanceof Element && event.target.closest(".vgt__resizeHandle"))
              return;
            if (event.target instanceof Element && event.target.closest(".vgt__filterBtn"))
              return;
    ...
    _openFilterMenu(absCol, anchorEl) {
      ...
      if (!col || !this._filterMenu || !this._filterOp || !this._filterValueA || !this._filterValueB || !this._filterSortBtn)
        return;
    ...
    _syncFilterMenuInputs() {
      if (!this._filterOp || !this._filterValueB)
        return;
    ...
    _applyFilterMenu() {
      if (this._filterMenuCol < 0 || !this._filterOp || !this._filterValueA || !this._filterValueB || !this._filterSortBtn)
        return;
      const absCol = this._filterMenuCol | 0;
    ...
    _setFilterSortState(state) {
      if (!this._filterSortBtn)
        return;
      const safeState = state === "asc" || state === "desc" ? state : "none";
      this._filterSortBtn.dataset.state = safeState;
      this._filterSortBtn.textContent = safeState === "asc" ? "Sort: ascending" : safeState === "desc" ? "Sort: descending" : "Sort: none";
    }
    _onContextMenu(event) {
      ...
      if (!(target instanceof Element))
        return;
      if (!this._isContextMenuSelectionTarget(target))
        return;
      event.preventDefault();
      this._openContextMenu(event.clientX, event.clientY);
    }
    _isContextMenuSelectionTarget(target) {
      return Boolean(target.closest(".vgt__cell") || target.closest(".vgt__rowBumper") || target.closest(".vgt__hcell") || target.closest(".vgt__headBumper"));
    }
    ...
    _windowPointerDown(event) {
      ...
      if (!(target instanceof Element))
        return;
      if (this._contextMenu?.dataset.open === "1") {
        if (!target.closest(".vgt__ctxMenu"))
          this._closeContextMenu();
      }
      if (this._filterMenuCol >= 0) {
      ...
      if (this._hasSelection() && !target.closest(".vgt__cell") && !target.closest(".vgt__rowBumper") && !target.closest(".vgt__hcell") && !target.closest(".vgt__headBumper") && !target.closest(".vgt__copyFabGroup") && !target.closest(".vgt__ctxMenu")) {
        this._clearSelection();
      }
    }
    _renderBody() {
      ...
      if (this._maxScrollPx() <= 0)
        this._scrollPx = 0;
      if (this._maxScrollXPx() <= 0)
        this._scrollXPx = 0;
      ...
      this._rowsInner.style.transform = "translate(" + -this._scrollXPx + "px, " + (-subPx - appliedRowOverscan * rowHeight) + "px)";
      ...
      for (let i = 0;i < this._rowEls.length; i += 1) {
        ...
        const row = this._mode === "server" ? this._chunkRows.get(baseIndex) : this._rows[baseIndex] || [];
        const isChunkLoading = this._mode === "server" && !row;
    ...
    _renderStatus() {
      const totalRows = this._mode === "server" ? this._chunkTotalRows : this._rows.length;
      ...
      if (shownRows !== totalRows)
        status += ` (filtered from ${totalRows})`;
      if (this._mode === "server" && this._chunkPending.size > 0)
        status += ` | loading ${this._chunkPending.size} page(s)`;
      if (totalCols > 0)
        status += ` | cols ${range.from}-${range.to} of ${totalCols}`;
    ...
    _startHeaderResize(event, colIndex) {
      if (colIndex < 0 || colIndex >= this._colWidths.length)
        return;
      event.preventDefault();
      event.stopPropagation();
      ...
      const onMove = (moveEvent) => {
        ...
        const nextWidth = this._clamp(Math.floor(startWidth + (moveEvent.clientX - startX)), this._minColWidth, this._maxColWidth);
        if (nextWidth === this._colWidths[colIndex])
          return;
    ...
    _headerPointerStart(event, headerCell) {
      ...
      const updateFromPointer = (shouldScheduleRaf) => {
        ...
        state.vScrollX = this._edgeVelocityX(rect, state.lastClientX, edgeThreshold, maxEdgeScrollPerFrame);
        const nextCol = this._colFromHeaderClientPointClamped(state.lastClientX);
        if (nextCol >= 0)
          this._selectColumnRange(startCol, nextCol);
      ...
      const onFrame = () => {
        ...
        if (!state.active)
          return;
        if (state.vScrollX !== 0)
          this._setScrollXPx(this._scrollXPx + state.vScrollX);
    ...
    _rowBumperPointerStart(event) {
      ...
      const updateFromPointer = (shouldScheduleRaf) => {
        ...
        state.vScroll = this._edgeVelocityY(rect, state.lastClientY, edgeThreshold, maxEdgeScrollPerFrame);
        const nextRow = this._rowFromBumperClientPointClamped(state.lastClientY);
        if (nextRow >= 0)
          this._selectRowRange(startRow, nextRow);
      ...
      const onFrame = () => {
        ...
        if (!state.active)
          return;
        if (state.vScroll !== 0)
          this._setScrollPx(this._scrollPx + state.vScroll);
        updateFromPointer(true);
      };
      const onMove = (moveEvent) => {
        moveEvent.preventDefault();
    ...
    _cancelActivePointerGesture() {
      ...
      if (gesture.rafId)
        window.cancelAnimationFrame(gesture.rafId);
    ...
    _startSelectionDrag(event, startCell) {
      ...
      const updateFromPointer = (shouldScheduleRaf) => {
        const rect = this._rowsHost.getBoundingClientRect();
        state.vScrollX = this._edgeVelocityX(rect, state.lastClientX, edgeThreshold, maxEdgeScrollPerFrame);
        state.vScrollY = this._edgeVelocityY(rect, state.lastClientY, edgeThreshold, maxEdgeScrollPerFrame);
        ...
        if (shouldScheduleRaf && state.rafId === 0 && (state.vScrollX !== 0 || state.vScrollY !== 0)) {
          state.rafId = window.requestAnimationFrame(onFrame);
        }
      };
      const onFrame = () => {
        ...
        if (state.vScrollX !== 0)
          this._setScrollXPx(this._scrollXPx + state.vScrollX);
        if (state.vScrollY !== 0)
          this._setScrollPx(this._scrollPx + state.vScrollY);
        updateFromPointer(true);
      };
      const onMove = (moveEvent) => {
        moveEvent.preventDefault();
    ...
    _startTouchGesture(event, startCell) {
      ...
      const onFrame = () => {
        state.rafId = 0;
        if (!state.active || gesture.mode !== "select")
          return;
        if (state.vScrollX !== 0)
          this._setScrollXPx(this._scrollXPx + state.vScrollX);
        if (state.vScrollY !== 0)
      ...
      if (gesture.mode === "pending") {
        gesture.timer = window.setTimeout(() => {
          if (this._activePointerGesture !== gesture || gesture.mode !== "pending")
            return;
          beginSelectionMode();
        }, this._longPressMs);
      } else {
      ...
      const cleanup = () => {
        ...
        this._rowsHost.style.touchAction = priorTouchAction;
        this._rowsHost.classList.remove("vgt__rows--dragging", "vgt__rows--selecting");
        if (this._activePointerGesture === gesture)
          this._activePointerGesture = null;
        try {
      ...
      onMove = (moveEvent) => {
        ...
        if (gesture.mode === "pending") {
          if (dist < this._touchMoveThreshold)
            return;
          gesture.mode = "scroll";
          if (gesture.timer)
            window.clearTimeout(gesture.timer);
    ...
    _cellFromElement(element) {
      ...
      if (!cellEl || !this._rowsHost.contains(cellEl))
        return null;
      ...
      if (!Number.isFinite(viewRow) || viewRow < 0 || !Number.isFinite(colIndex) || colIndex < 0)
        return null;
    ...
    _rowFromBumperElement(element) {
      ...
      if (!bumperEl || !this._rowBumpers.contains(bumperEl))
        return -1;
      const viewRow = Number(bumperEl.dataset.viewRow);
      if (!Number.isFinite(viewRow) || viewRow < 0)
        return -1;
      return viewRow;
    }
    _selectAllCells() {
    ...
    _isCellSelected(viewRow, colIndex) {
      ...
      if (!range)
        return false;
      return viewRow >= range.rowMin && viewRow <= range.rowMax && colIndex >= range.colMin && colIndex <= range.colMax;
    }
    ...
    _isAllSelected() {
      ...
      const rowCount = this._viewCount | 0;
      const colCount = this._columns.length | 0;
      if (!range || rowCount <= 0 || colCount <= 0)
        return false;
      return range.rowMin === 0 && range.rowMax === rowCount - 1 && range.colMin === 0 && range.colMax === colCount - 1;
    }
    ...
    _syncMobileCopyButton() {
      if (!this._copyFabGroup)
        return;
      const hasSelection = this._hasSelection();
      const isGestureActive = this._pointerSelecting || this._rowsHost.classList.contains("vgt__rows--dragging") || this._rowsHost.classList.contains("vgt__rows--selecting") || this._rowBumpers.classList.contains("vgt__rows--selecting");
      const shouldShow = this._isCoarsePointer() && hasSelection && !isGestureActive;
      this._copyFabGroup.dataset.show = shouldShow ? "1" : "0";
      if (!shouldShow)
        this._copyFab.dataset.copied = "0";
      if (!shouldShow && this._copyFabWithHeaders)
        this._copyFabWithHeaders.dataset.copied = "0";
    }
    _flashMobileCopyButton(button) {
      const targetBtn = button || this._copyFab;
      if (!targetBtn || this._copyFabGroup?.dataset.show !== "1")
        return;
      if (this._mobileCopyResetTimer) {
        window.clearTimeout(this._mobileCopyResetTimer);
        this._mobileCopyResetTimer = 0;
    ...
    _isEditableTarget(target) {
      ...
      return tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT" || Boolean(target.closest("[contenteditable]"));
    }
    _onCopy(event) {
      if (!this._hasSelection())
        return;
...
`),
    ...
    _selectionRowsForClipboard(includeHeaders = false) {
      ...
      for (let viewRow = rowMin;viewRow <= rowMax; viewRow += 1) {
        ...
        if (baseIndex < 0)
          continue;
        const sourceRow = this._mode === "server" ? this._chunkRows.get(baseIndex) : this._rows[baseIndex];
        const row = sourceRow || [];
        const rowOut = [];
        for (let col = colMin;col <= colMax; col += 1) {
          const value = row[col];
    ...
    _selectionRowsToHtml(rows, hasHeaderRow) {
      ...
      const tableStyle = "border-collapse:collapse;font-family:Arial,sans-serif;font-size:13px;line-height:1.4;color:#0f172a;background:#ffffff;table-layout:auto;";
      const thStyle = "border:1px solid #d0d7de;padding:6px 10px;background:#f6f8fa;font-weight:600;text-align:left;white-space:nowrap;";
      const tdStyle = "border:1px solid #d0d7de;padding:6px 10px;text-align:left;vertical-align:top;white-space:nowrap;";
...
`);
    ...
    _requestChunk(start, endExclusive, reason) {
      ...
      if (safeEnd <= safeStart)
        return;
      if (!this._windowHasMissingRows(safeStart, safeEnd))
        return;
      const key = safeStart + ":" + safeEnd;
      if (this._chunkPending.has(key))
        return;
      ...
      this._host.dispatchEvent(new CustomEvent("vgt:server-page-request", { detail: request }));
      let callbackResult;
      if (this._onChunkRequest) {
        try {
          callbackResult = this._onChunkRequest(request);
        } catch (err) {
          console.error("VirtualGridTable onServerPageRequest failed", err);
        }
      }
      const providerResult = this._fetchChunk ? this._fetchChunk(request) : callbackResult;
      if (providerResult == null)
        return;
      Promise.resolve(providerResult).then((response) => {
        this._consumeChunkResponse(response, safeStart, key);
      }).catch((err) => {
        console.error("VirtualGridTable fetchPage failed", err);
        this._chunkPending.delete(key);
        this._renderStatus();
      });
    }
    _consumeChunkResponse(response, defaultStart, pendingKey) {
      ...
      if (Array.isArray(response)) {
        this.setServerRows(defaultStart, response);
      ...
      if (response && typeof response === "object") {
        ...
        this.setServerRows(start, rows, totalRows);
        this._chunkPending.delete(pendingKey);
        this._renderStatus();
        return;
      }
    ...
    _clearPendingWindowsForRange(start, endExclusive) {
      ...
      for (const key of Array.from(this._chunkPending)) {
        ...
        if (hi > chunkLo && lo < chunkHi)
          this._chunkPending.delete(key);
      }
    }
    _windowHasMissingRows(start, endExclusive) {
      for (let i = start;i < endExclusive; i += 1) {
        if (!this._chunkRows.has(i))
          return true;
      }
      return false;
    }
    ...
    _edgeVelocityX(rect, clientX, edgeThreshold, maxEdgeScrollPerFrame) {
      return this._edgeVelocity(clientX, rect.left + edgeThreshold, rect.right - edgeThreshold, edgeThreshold, maxEdgeScrollPerFrame);
    }
    _edgeVelocityY(rect, clientY, edgeThreshold, maxEdgeScrollPerFrame) {
      return this._edgeVelocity(clientY, rect.top + edgeThreshold, rect.bottom - edgeThreshold, edgeThreshold, maxEdgeScrollPerFrame);
    }
    _clamp01(x) {
  ...
  (function autoInitVgtDemo() {
    ...
    if (!host)
      return;
    const autoOptions = window.__VGT_AUTO_INIT_OPTIONS__ && typeof window.__VGT_AUTO_INIT_OPTIONS__ === "object" ? window.__VGT_AUTO_INIT_OPTIONS__ : {};
    ...
    if (grid._opts.demo_mode === "server") {
      ...
      grid.setServerPaging({
        ...
        async fetchPage(request) {
          ...
          if (!response.ok) {
            throw new Error(`Server page fetch failed (${response.status}): ${url}`);
          }
