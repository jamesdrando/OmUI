 ...
 * - clearSort()
 * - setServerPaging({ columns, totalRows, pageSize, onServerPageRequest?, fetchPage? })
 * - setServerRows(startIndex, rows, totalRows?)
 * - setServerRowCount(totalRows)
 * - clearServerCache()
 * - getOffsets()
 * - destroy()
 */
class VirtualGridTable {
  ...
  constructor(containerId, options = {}) {
    ...
    this._onChunkRequest =
      typeof this._opts.onServerPageRequest === "function" ? this._opts.onServerPageRequest : null;
  ...
  setServerPaging(config = {}) {
    ...
    if (typeof next.onServerPageRequest === "function") this._onChunkRequest = next.onServerPageRequest;
    if (typeof next.fetchPage === "function") this._fetchChunk = next.fetchPage;
  ...
  setServerRows(startIndex, rows, totalRows) {
    ...
    if (Number.isFinite(totalRows)) this.setServerRowCount(totalRows, false);
  ...
  _renderStatus() {
    ...
    if (this._mode === "server" && this._chunkPending.size > 0) status += ` | loading ${this._chunkPending.size} page(s)`;
