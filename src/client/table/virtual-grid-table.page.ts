...
(function initDashboardTablePage() {
  ...
  function syncOptionLabels(options) {
    for (const key of Object.keys(optionLabels)) delete optionLabels[key];
    for (const option of options) optionLabels[option.key] = option.label;
  }
  ...
  function toQueryFilters(request, columns) {
    ...
    if (request.query && request.query.trim()) {
      if (request.searchColumn >= 0) {
        ...
        if (col?.key) filters.push({ field: col.key, op: "contains", value: request.query.trim() });
      } else {
        const first = columns[0];
        if (first?.key) filters.push({ field: first.key, op: "contains", value: request.query.trim() });
      }
    }
  ...
  async function loadDataset(key) {
    ...
    try {
      ...
      const selected = list.some((item) => item.key === key)
        ? key
        : list.some((item) => item.key === config.defaultDataset)
      ...
      if (!descriptor) throw new Error("Dataset metadata not found for " + selected);
